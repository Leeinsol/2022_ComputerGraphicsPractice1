...
bool ColorShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, float bright)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, bright);
	if (!result)
	{
		return false;
	}
	...
	const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC brightBufferDesc;
	...
	polygonLayout[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;	//12바이트
	...
	polygonLayout[1].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;	//16바이트
	...
	brightBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	brightBufferDesc.ByteWidth = sizeof(MatrixBufferType);
	brightBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	brightBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	brightBufferDesc.MiscFlags = 0;
	brightBufferDesc.StructureByteStride = 0;


	result = device->CreateBuffer(&brightBufferDesc, NULL, &m_brightBuffer);
	if (FAILED(result))
	{
		return false;
	}
	...
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, float bright)
{
	...
	result = deviceContext->Map(m_brightBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
	{
		return false;
	}

	dataPtr2 = (BrightnessType*)mappedResource.pData;

	dataPtr2->brightness = bright;
	
	deviceContext->Unmap(m_brightBuffer, 0);

	bufferNumber = 0;

	deviceContext->PSSetConstantBuffers(bufferNumber, 1, &m_brightBuffer);
